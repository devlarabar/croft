...
- **Bedrock**: the access key ID and secret of an IAM user allowed to
  invoke models. Claude uses `eu.*` inference profiles; the 5-family models
  need a credential saved with region `eu-west-1`, the 4-x ones
  `eu-central-1`. GPT-5.6 uses bare `openai.*` model IDs through the
  `us-east-1` Bedrock Mantle endpoint, regardless of the saved region.
...
1. **Pick a region and enable model access.** Open the
   ...
   the Claude and OpenAI GPT models you plan to use → submit. Anthropic asks
   for a short use-case form; approval is usually immediate. Access is
   per-region.
...
Claude uses Bedrock Runtime's `eu.anthropic.…` inference profiles. GPT-5.6
matches Journalia's Bedrock Mantle setup and uses `openai.gpt-5.6-*` IDs.
They are defined in `packages/core/src/llm/adapters/bedrock.ts`.
