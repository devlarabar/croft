...
test("public endpoints stay public while all unknown dashboard paths require sign-in", async () => {
  ...
  for (const path of ["/runs", "/runs/9fe4d6f2-c580-4acf-94c3-c5e5a994ccdb/logs", "/runs/9fe4d6f2-c580-4acf-94c3-c5e5a994ccdb/logs.json", "/settings", "/api/docs", "/api/local-runs", "/oauth/openai", "/unknown", "/api/forms/settings"]) {
...
test("device login allows authenticated reads and same-origin checks only", async () => {
  process.env.DEV_NO_AUTH = "1";
  const url = origin + "/oauth/openai";
  for (const method of ["GET", "POST"]) {
    const response = await proxy(new NextRequest(url, { method, headers: { origin } }));
    assert.equal(response.headers.get("x-middleware-next"), "1");
    assert.equal(response.headers.get("cache-control"), "private, no-store");
  }
  assert.equal((await proxy(new NextRequest(url, { method: "POST", headers: { origin: "https://other.test" } }))).status, 403);
});
