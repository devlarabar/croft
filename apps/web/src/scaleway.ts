...
export async function startJob(env: Record<string, string>): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Scaleway start job failed (HTTP ${res.status}).`);
...
export async function getJobRunState(jobRunId: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Scaleway get job run failed (HTTP ${res.status}).`);
...
export async function stopJob(jobRunId: string): Promise<void> {
  ...
  if (!res.ok) throw new Error(`Scaleway stop job failed (HTTP ${res.status}).`);
}
