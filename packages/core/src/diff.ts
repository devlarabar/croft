...
// A repeat review diffs previousHead...head; after a rebase that compare
// includes mainline commits from other PRs. Keep only sections for files the
// PR itself changes.
export function restrictDiffToFiles(diff: string, allowed: Set<string>): string {
  const sections = diff.split(/^(?=diff --git )/m);
  return sections
    .filter((section) => {
